"""
PluginBase class for plugin development framework in K8s-Gen DSL.

This module provides the base classes and interfaces for developing
custom plugins that extend the K8s-Gen DSL functionality.
"""
...
class PluginBase(ABC):
    """
    Base class for all K8s-Gen plugins.
    ...
    def is_compatible(self, version: str) -> bool:
        """
        Check if plugin is compatible with given K8s-Gen version.
        
        Args:
            version: K8s-Gen version
...
class BuilderPlugin(PluginBase):
    ...
    the core K8s-Gen DSL with new resource types.
    """
...
class LabelTransformer(TransformerPlugin):
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        self._standard_labels = config.get("labels", {
            "app.kubernetes.io/managed-by": "k8s-gen",
            "app.kubernetes.io/created-by": "k8s-gen-dsl"
        })
